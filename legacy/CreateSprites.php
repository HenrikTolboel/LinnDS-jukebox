...
function CreateSprites(&$Menu)
{
    ...
    {
	echo $i .": " . $manifest->PresetImage[$i] . $NL;
	$img = rawurldecode($manifest->PresetImage[$i]);
	echo $i . ": " . $img .$NL;
	if (strlen($img) > 4 && file_exists("../" . $img))
	{
	    $sourcefile = '../' . $img;
	    $newfile = sprintf("site/folder/folder_%04d.jpg", $i);
	}
	else
	{
	    $sourcefile = "grey.jpg";
    ...
    echo "Run montage -background transparent -tile " . $TileW . "x" . $TileH . " -geometry 80x80+1+1 site/folder/folder_* site/sprites/sprite.jpg\n";
    echo "Run montage -background transparent -tile " . $TileW . "x" . $TileH . " -geometry 160x160+1+1 site/folder/folder_* site/sprites/sprite@2x.jpg\n";
    ...
    file_put_contents("site/sprites/sprites.css", $css);
    ...
    file_put_contents("site/sprites/sprites@2x.css", $css);
}
